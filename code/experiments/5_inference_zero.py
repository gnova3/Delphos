...
from inference.inference_zero_shot_learning import run_zero_shot_inference
...
for seed in seeds:
    print(f"\n{'='*50}")
    print(f"Starting Inference for Trained Agent SEED: {seed}")
    print(f"{'='*50}\n")
    ...
    for cp_name in possible_checkpoint_names:
        ...
        print(f"\nFound checkpoint: {cp_name}. Running Inference...")
        ...
        for task in inference_tasks:
            ...
            for strat in strategies:
                ...
                # Create a subfolder per strategy inside the task results
                output_dir = cp_dir / "inference_results" / f"task_{task.id}" / strat_name
                
                try:
                    output_dir.mkdir(parents=True, exist_ok=True)
                    df = run_zero_shot_inference(
                        trainer=trainer,
                        task=task,
                        n_models=100,
                        max_attempts=500,
                    ...
                    df.to_csv(output_dir / "zero_shot_results.csv", index=False)
                    print(f"        Results saved to {output_dir / 'zero_shot_results.csv'} ({len(df)} unique models)")
                except Exception as e:
                    print(f"        Error running inference on {task.name} with {strat_name}: {e}")
