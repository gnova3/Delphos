...
Delphos is a Deep Q-Network (DQN) agent designed to
...
from agent.encoder import DeepSetEncoder, InteractionAwareEncoder, TermEncoder, ZStateConfig
...
class Delphos(nn.Module):
    ...
    def encode_specification(self, specification: torch.LongTensor, runtime: Runtime,) -> torch.Tensor:
        """
        Encode a specification tensor into a latent state representation.
        """
    ...
    def select_action(
        ...
        Parameters
        ----------
        specification: Current specification tensor.
        runtime: Runtime object.
        visited_specifications: Specification keys already visited during the current episode.
        epsilon: Exploration probability.
        boltzmann: Whether Boltzmann exploration should be used.
        temperature: Softmax temperature.

        Returns
        -------
        action_index: Selected catalogue action.
    ...
    def save_checkpoint(self, path: str, save_mode: str = "full", extra_metadata: Optional[dict] = None) -> None:
        """
        Save Delphos checkpoint.

        Parameters
        ----------
        path: Output checkpoint path.
        save_mode: "full" -> encoder + policy + target + optimizer, "encoder" -> encoder only, "policy" -> policy only
        extra_metadata: Optional user metadata.
        """
    ...
    def load_checkpoint(self, path: str, load_mode: str = "resume",) -> dict:
        """
        Load Delphos checkpoint.

        Parameters
        ----------
        path: Checkpoint path.
        load_mode: "resume", "encoder", "policy" or "transfer"

        Returns
        -------
        dict: Stored checkpoint metadata.
        """
